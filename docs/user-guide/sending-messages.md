...
# Enable link preview
client.send_message(
    recipient, "Check this out: https://github.com/krypton-byte/neonize", link_preview=True
)
```
...
client.send_message(recipient, "Secret mention here", ghost_mentions="@1234567890 @0987654321")
```
...
```python
# From file path
client.send_image(recipient, "path/to/image.jpg", caption="Beautiful sunset! 🌅")

# From URL
client.send_image(recipient, "https://example.com/image.jpg", caption="Check this image")
...
# View once image
client.send_image(recipient, "secret.jpg", caption="This will disappear", viewonce=True)
```
...
```python
# Video message
client.send_video(recipient, "video.mp4", caption="Amazing video! 🎥")

# Video as GIF
client.send_video(recipient, "animation.mp4", gifplayback=True)

# From actual GIF file
client.send_video(recipient, "animation.gif", is_gif=True, gifplayback=True)
```
...
```python
# Audio message
client.send_audio(recipient, "audio.mp3")
...
client.send_document(
    recipient, "data.csv", filename="data.csv", mimetype="text/csv", caption="Data export"
)
```
...
client.send_album(recipient, files, caption="My vacation photos! 🏖️")
```
...
```python
client.send_contact(recipient, contact_name="John Doe", contact_number="1234567890")
```
...
poll = client.build_poll_vote_creation(
    "What's your favorite color?", ["Red 🔴", "Blue 🔵", "Green 🟢", "Yellow 🟡"], VoteType.SINGLE
)
...
poll = client.build_poll_vote_creation(
    "Select your hobbies:", ["Reading 📚", "Gaming 🎮", "Sports ⚽", "Music 🎵"], VoteType.MULTIPLE
)
...
reaction = client.build_reaction(
    chat=chat_jid, sender=sender_jid, message_id="MESSAGE_ID", reaction="❤️"
)
...
client.reply_message(
    "Check this: https://github.com/krypton-byte/neonize", event, link_preview=True
)
```
...
client.reply_message("This is a private reply", event, reply_privately=True)
```
...
# Edit sent message
client.edit_message(chat_jid, message_id="MSG_ID", new_message=Message(conversation="Updated text"))
```
...
client.revoke_message(chat_jid, sender_jid, message_id="MSG_ID")
```
...
```python
# Build image message
image_msg = client.build_image_message("photo.jpg", caption="Custom image", viewonce=False)
...
client.send_message(recipient, "Secure message", add_msg_secret=True)
```
...
reply_msg = client.build_reply_message(
    "This is my reply", quoted=original_message, link_preview=True
)
...
for result in results:
    if result.is_in:
        ...
        client.send_message(build_jid(result.jid.User), "Hello!")
```
...
async def send_bulk_messages(recipients, message):
    ...
    tasks = [client.send_message(build_jid(number), message) for number in recipients]
